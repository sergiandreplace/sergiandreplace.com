...
As you may see, we are just placing an element of type PlanetRow and importing the file PlanetRow.dart. Now we can create it, again, with a simple base:
...
* ```EdgeInsets.symmetric(vertical, horizontal)```: allows us to specifc top/bottom and/or left/right with a single value.
